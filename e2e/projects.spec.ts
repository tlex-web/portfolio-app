...
test.describe('Projects Page', () => {
  ...
  test('should navigate to project detail page', async ({ page }) => {
    ...
    // Click first project link (Playwright auto-waits for visibility)
    const projectLink = page.getByRole('link', { name: /View details for/i }).first();
    await projectLink.click();

    // Verify we're on the project detail page (allow time for client-side navigation)
    await expect(page).toHaveURL(/\/projects\/[\w-]+/, { timeout: 15000 });
  });
